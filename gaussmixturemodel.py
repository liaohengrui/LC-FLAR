...
class DiscretizedGaussianMixtureModel(nn.Module):
    def __init__(self, mean, log_sigma, mixture_weights,
                 autoregression_coefficients=None, *args, **kwargs):
    ...
    def forward(self, input):
        """
        input: (N, 3, H, W), assumed in [0, 1]
        """
        N, C, H, W = input.shape
        half = 0.5 / 255.0

        x = torch.reshape(input, (N, C, 1, H, W)) \
            + torch.zeros(N, C, self.mix_num, H, W, device=input.device)

        # autoregressive mean adjustment
        if self.channel_ag:
            ...
            self.mean = torch.cat((m1, m2, m3), dim=1)

        centered_x = x - self.mean
        inv_sigma = torch.exp(-self.log_sigma)

        # ---- discretized Gaussian CDF ----
